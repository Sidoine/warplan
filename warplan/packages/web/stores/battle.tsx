...
const BattleStoreContext = createContext<BattleStore | null>(null);

export function BattleStoreProvider({
    children,
}: {
    ...
    const [battleStore] = useState(
        () => new BattleStore(dataStore, armyListStore)
    );
    return (
        <BattleStoreContext.Provider value={battleStore}>
            {children}
...
export function useBattleStore() {
    const dataStore = React.useContext(BattleStoreContext);
    if (!dataStore) {
        throw new Error(
            "useBattleStore must be used within a BattleStoreProvider"
        );
    }
    return dataStore;
}

...
async function load() {
    try {
        ...
        let result = `import { DataStore, GrandAlliance, WarscrollInterface, Box, AbilityCategory } from "./unit";
...
function compoundKeywordsToString(keywords: def.CompoundKeyword[]) {
    return `[${keywords
        .map(
            (y) =>
                `[${y.keywords.map((z) => `"${z.toUpperCase()}"`).join(", ")}]`
        )
        .join(", ")}]`;
}

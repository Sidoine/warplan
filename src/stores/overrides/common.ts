...
import { Phase, TargetType } from "../units";
...
export function overrideCommon(data: DataStoreImpl) {
    ...
    overrideAbility(data.abilities.suffocatingGravetidePulledToTheGrave, x => {
        x.effects = [{ phase: Phase.Hero, targetType: TargetType.Unit }];
    });
    overrideAbility(data.abilities.suffocatingGravetideRoilingBarricade, x => {
        x.effects = [{ phase: Phase.Shooting, targetType: TargetType.Unit }];
    });
    overrideAbility(data.abilities.suffocatingGravetideEmpoweredByShyish, x => {
        x.effects = [{ phase: Phase.Hero, targetType: TargetType.Model }];
    });

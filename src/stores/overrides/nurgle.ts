...
import { Ability, Scenery, TargetType, Phase, SubPhase, AbilityCategory } from "../units";
import { override, artifactWithKeywordAvailable, overrideAbility, addAbilityEffect } from "./tools";
...
function addCommandAbilities(data: DataStoreImpl):void {
    overrideAbility(data.extraAbilities.nurgleCommandTraitsPestilentBreath.ability, x => {
        x.description = "At the start of your shooting phase, pick an enemy unit withing 6 inches of your general. Roll a dice for each model within 6 inches of your general. The unit suffers a mortal wound for each roll of 5+.";
        x.effects = [{
...
function addSpells(data: DataStoreImpl):void {
    overrideAbility(data.extraAbilities.nurgleLoreOfFoulnessPlagueSquall.ability, x => {
        x.description = "Casts on a 6+. If successful, roll 7D6. For each 6, pick an enemy unit (no line of sight needed) and deal D3 Mortal Wounds to it. You can only pick each enemy unit once.",
        x.effects = [{
            targetType: TargetType.Enemy,
            phase: Phase.Hero
        }];
    ...
    overrideAbility(data.extraAbilities.nurgleLoreOfMalignanceBladesOfPutrefaction.ability, x => {
        x.category = AbilityCategory.Spell;
        x.spellCastingValue = 7;
        x.description = "Casts on a 7+, affects a friendly unit within 14\". 6+ To Hit deal a Mortal Wound in addition to other damage, not specifying melee or shooting";
        x.effects = [{
            targetType: TargetType.Friend,
            phase: Phase.Hero
        }];
...
function addArtefact(data: DataStoreImpl):void {
    ...
    addAbilityEffect(data.extraAbilities.nurgleDaemonicBoonsTheWitherstave.ability, {
        targetType: TargetType.Enemy,
        phase: Phase.Combat
    });
    ...
    addAbilityEffect(data.extraAbilities.nurgleArtefactsOfContagionRustfang.ability, {
        targetType: TargetType.Enemy,
        phase: Phase.Hero
    });

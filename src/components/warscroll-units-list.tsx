...
export class WarscrollUnitsList extends React.Component<WarscrollUnitsListProps, {}> {
    ...
    @action private handleOpenWarscroll = (unit: WarscrollUnit) => this.warscrollOpen = unit;
    ...
    private renderModelName = (unit: WarscrollUnit) => {
        return <><div>{unit.unit.model.name} 
        <IconButton onClick={() => this.handleOpenWarscroll(unit)} size="small"><VisibilityIcon/></IconButton></div>
        ...
        <div>
            ...
            <div>{(!unit.isAllied && unit.isLeader) && <><Checkbox checked={unit === this.props.warscrollStore!.warscroll.general} onChange={(_, c) => this.toggleGeneral(unit, c)} ></Checkbox>General</> } 
            {unit.isAllied && <>Allied</> }</div>
        </div></>;
    }

    private renderModelOptions = (unit: WarscrollUnit) => {
        return <> {unit.models.map(x => this.renderModel(unit, x))}
            {unit.availableOptions.length > 0 && <AddButton columns={modelColumns} options={unit.availableOptions} onChange={this.handleAddModel(unit)} />}
        </>;
    }
    
    private renderModel(unit: WarscrollUnit, model: WarscrollModel) {
        return <div key={model.id}>
            <NumberControl value={model.count} onChange={this.handleModelCountChange(unit, model)}/>
    ...
    private renderExtras = (unit: WarscrollUnit) => {
        return <>{ join(unit.extraAbilities.map(x => <span key={x.id}>
    ...
    render() {
        ...
        return <Card>
            ...
            <CardContent>
            <Modal open={this.warscrollOpen !== null} onClose={this.handleCloseWarscroll}>
                <><UnitWarscroll wu={this.warscrollOpen}/></>
            </Modal>
            
                <ResponsiveTable columns={this.columns} rows={warscroll.units} />
            </CardContent>
            <CardActions>

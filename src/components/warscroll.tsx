...
import { AllAbilities, useWarscrollStyles } from "../atoms/warscroll-components";
import { UnitWarscroll } from "./unit-warscoll";
import { Table, TableRow, TableCell, TableBody  } from "@material-ui/core";
import { useStores } from "../stores";

function BattalionView({battalion } : { battalion: WarscrollBattalionInterface }) {
    ...
    return <div className={classes.warscroll}>
        <div className={classes.header}>
            <div className={`${classes.title} ${classes.battalion}`}>
                ...
                <div className={classes.count}>{battalion.battalion.points} points</div>
            </div>
        </div>
        <div className={classes.flavor}>{battalion.battalion.description}</div>
...
export function Warscroll() {
    ...
    return <div>
        ...
        {warscrollStore.armyOptions && w.armyOption && <div>{warscrollStore.armyOptions.name}: {w.armyOption.name}</div>}
    ...
    {w.battalions.length > 0 &&
        <><h1>Battalions</h1>
                {
                    w.battalions.map(x => <BattalionView key={x.id} battalion={x} />)
                }
        </>}    
    ...
    {w.endlessSpells.length > 0 &&
        <section> 
            ...
            <Table>
                <TableBody>
                    {
                        w.endlessSpells.map(x => <EndlessSpellView scenery={x} key={x.id} />)
                    }
                </TableBody>
            </Table>
        </section>}    
